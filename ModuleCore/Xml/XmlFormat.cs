...
namespace DNNGo.Modules.DNNGalleryPro
{
    ...
    public class XmlFormat
    {
        ...
        /// <param name="list"></param>
        /// <returns></returns>
        public String ToXml<T>(List<T> list, List<GallerySettingsEntity> Settings, List<GalleryGroupEntity> Groups)
        {
            StringBuilder sb = new StringBuilder();
            ...
            using (StreamReader sr = new StreamReader(XmlUrl))
            {
                ...
                {
                    Type t = typeof(GalleryGroupEntity);
                    PropertyInfo[] Propertys = t.GetProperties();

                    GroupSB.AppendFormat("  <{0}List>", t.Name).AppendLine();


                    //先循环数据列表
                    foreach (GalleryGroupEntity ItemInfo in Groups)
                    {
                        GroupSB.AppendFormat("    <{0}Item>", t.Name).AppendLine();
                        //再循环字段列表
                        foreach (PropertyInfo Property in Propertys)
                        {
                            object o = Property.GetValue(ItemInfo, null);
                            GroupSB.AppendFormat("      <{0}><![CDATA[{1}]]></{0}>", Property.Name, FormatValueToString(o, Property.PropertyType)).AppendLine();
                        }
                        GroupSB.AppendFormat("    </{0}Item>", t.Name).AppendLine();
                    }
                    GroupSB.AppendFormat("  </{0}List>", t.Name).AppendLine();

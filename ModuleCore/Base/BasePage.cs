...
namespace DNNGo.Modules.DNNGalleryPro
{
    public class BasePage : DotNetNuke.Framework.PageBase
    {
        ...
        private Hashtable _DNNGalleryPro_Settings = new Hashtable();
        /// <summary>
        /// 获取模块配置(可以获取其他模块配置)
        /// </summary>
        public Hashtable DNNGalleryPro_Settings
        {
            get
            {
                if (!(_DNNGalleryPro_Settings != null && _DNNGalleryPro_Settings.Count > 0))
                {
                    if (Settings_CopyOfOtherModule && Settings_ModuleID != ModuleId)
                    {
                        ModuleInfo m = ModuleController.Instance.GetModule(Settings_ModuleID, Settings_TabID,true);
                        _DNNGalleryPro_Settings = m.ModuleSettings;
                    }
                    else
                    {
                        _DNNGalleryPro_Settings = Settings;
                    }
                }
                return _DNNGalleryPro_Settings;
            }
        }
        ...
        public object ViewXmlSetting(String Name, object DefaultValue)
        {
            ...
            return DNNGalleryPro_Settings[SettingKey] != null ? DNNGalleryPro_Settings[SettingKey] : DefaultValue;
        }

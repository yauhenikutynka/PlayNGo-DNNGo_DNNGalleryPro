...
namespace DNNGo.Modules.DNNGalleryPro
{
    ...
    public class Common
    {
       ...
       /// <returns></returns>
        ...
        /// <returns></returns>
        public static bool CompareVersion(string old_version, string new_version)
        {
            try
            {
                if (string.IsNullOrEmpty(old_version) || string.IsNullOrEmpty(new_version))
                    return false;
        ...
        /// <returns></returns>
        public static String ConvertFileSize(Int32 FileSize)
        {
            String strFileSize = "0 KB";
            if (FileSize > (1024 * 1024))
            {
                strFileSize = String.Format("{0:N} GB", Convert.ToSingle(FileSize) / (1024 * 1024));
            }
            else if (FileSize > 1024)
            {
                strFileSize = String.Format("{0:N} MB", Convert.ToSingle(FileSize) / Convert.ToSingle(1024));
            }
            else
            {
                strFileSize = String.Format("{0} KB", FileSize);
            }
            return strFileSize;
        }
        #endregion
        ...
        /// <param name="PageSize">每页数</param>
        /// <returns>分页好的数据</returns>
        public static List<T> Split<T>(ArrayList list, Int32 PageIndex, Int32 PageSize)
        {
            ...
            return Split<T>(tempList.ToArray(), PageIndex, PageSize);
        }

...
namespace DNNGo.Modules.DNNGalleryPro
{
    ...
    public class ControlHelper
    {
        ...
        public Control ViewControl(SettingEntity FieldItem)
        {
            ...
            else if (FieldItem.ControlType == EnumControlType.DateTimePicker.ToString())
                ControlHtml = ViewCreateDateTimePicker(FieldItem, ControlName, ControlID);
        ...
        /// <param name="ControlName"></param>
        /// <returns></returns>
        public String ViewCreateDateTimePicker(SettingEntity FieldItem, String ControlName, String ControlID)
        {
            StringBuilder ControlHtml = new StringBuilder();//控件的HTML

            if (!String.IsNullOrEmpty(FieldItem.DefaultValue) && FieldItem.IsTime)
            {
                //判断是否需要把日期转换成带时区的
                DateTime oTime = xUserTime.UtcTime();
                string[] expectedFormats = { "G", "g", "f", "F" };
                if (DateTime.TryParseExact(FieldItem.DefaultValue, "MM/dd/yyyy HH:mm:ss", new CultureInfo("en-US", false), DateTimeStyles.AllowWhiteSpaces, out oTime))
                {
                    FieldItem.DefaultValue = xUserTime.LocalTime(oTime).ToString("MM/dd/yyyy HH:mm:ss", new CultureInfo("en-US", false));
                }
            }
            ...
            ControlHtml.Append("jQuery(document).ready(function(){").AppendLine();
            ControlHtml.Append("	    jQuery(function() {").AppendLine();
            ControlHtml.AppendFormat("		    jQuery(\"#{0}\").datetimepicker({{format:'MM/DD/YYYY HH:mm:ss'}});", ControlID).AppendLine();
            ControlHtml.Append("	    });").AppendLine();
            ControlHtml.Append("});").AppendLine();
            ControlHtml.Append("</script>");
            return ControlHtml.ToString();
        ...
        public String ViewCreateRichTextBox(SettingEntity FieldItem, String ControlName, String ControlID)
        {
            ...
            ControlHtml.AppendFormat(" class=\"form-control ckeditor_ input_text {0}\"", ViewVerification(FieldItem));

            ControlHtml.AppendFormat(" style=\"width:{0}px;height:{1}px;\" rows=\"{2}\"", FieldItem.Width, FieldItem.Rows * 20, FieldItem.Rows);
            //ControlHtml.AppendFormat(" style=\"width:{0}px;height:{1}px;\" rows=\"{1}\"", FieldItem.Width, FieldItem.Rows);
            ...
            ControlHtml.Append("jQuery(function ($) {").AppendLine();
            ControlHtml.AppendFormat(" CKEDITOR.replace('{0}', {{height:'{1}px'}});", ControlID, FieldItem.Rows * 20).AppendLine();
            ControlHtml.Append(" });").AppendLine();
        ...
        {
            return String.Format("<img src=\"{0}Effects/{1}/images/{2}\" class=\"img-responsive tooltips\" title=\"{3}\" data-placement=\"top\" data-rel=\"tooltip\" data-original-title=\"{3}\" />", BaseModule.ModulePath, BaseModule.Settings_EffectName, _Icon, _Text);
        }

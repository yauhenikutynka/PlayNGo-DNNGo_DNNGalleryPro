    ...
    public class Resource_jQueryFileUploadHandler : IHttpHandler
    {
        ...
        public Hashtable Settings
        {
            get
            {
                ...
                {
                    _settings = ModuleConfiguration.ModuleSettings;
                }
                return _settings;
            }
        }
		...
		private void DeleteFile (HttpContext context) {
            ...
            DNNGo_DNNGalleryPro_Files PhotoItem = DNNGo_DNNGalleryPro_Files.FindByKeyForEdit(PhotoID);
        ...
        /// <returns></returns>
        public String GetPhotoPath(DNNGo_DNNGalleryPro_Files media)
        {
		...
		private void ListCurrentFiles (HttpContext context) {
            ...
            //int RecordCount = 0;
            //qp.Where.Add(new SearchParam(DNNGo_DNNGalleryPro_Files._.AlbumID, WebHelper.GetStringParam(context.Request, "AlbumID", "0"), SearchType.Equal));

            //var files = DNNGo_DNNGalleryPro_Files.FindAll(qp, out RecordCount).Select(f => new Resource_FilesStatus(f, PortalSettings)).ToArray();
        ...
        /// 压缩图片
        /// </summary>
        /// <param name="fileStream">要压缩的流</param>
        /// <param name="newfilePath">压缩后的图片的路径</param>
        public void ChangeImageSize(HttpPostedFile fileStream, string newfilePath)
        {



            if (ViewSettingT<Boolean>("DNNGalleryPro_CompressionEnable", false))
            {
                Bitmap bmp = null;
                ...
                try
                {
                    bmp = new Bitmap(fileStream.InputStream);
                    ici = this.getImageCoderInfo(fileStream.ContentType);
                    ecd = Encoder.Quality;
                    eptS = new EncoderParameters(1);
                    ept = new EncoderParameter(ecd, ViewSettingT<long>("DNNGalleryPro_CompressionQuality", 90L));
                    eptS.Param[0] = ept;

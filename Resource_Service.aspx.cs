...
namespace DNNGo.Modules.DNNGalleryPro
{
    ...
    public partial class Resource_Service : BasePage
    {
        ...
        {
            QueryParam qp = new QueryParam();
            qp.Orderfld = DNNGo_DNNGalleryPro_Layer._.ID;
            qp.OrderType = 1;

            qp.PageIndex = WebHelper.GetIntParam(Request, "PageIndex", 1);
            qp.PageSize = WebHelper.GetIntParam(Request, "PageSize", Int32.MaxValue);
            qp.Where.Add(new SearchParam(DNNGo_DNNGalleryPro_Files._.Status, (Int32)EnumStatus.Activated, SearchType.Equal));
            qp.Where.Add(new SearchParam(DNNGo_DNNGalleryPro_Files._.PortalId, PortalId, SearchType.Equal));


            int RecordCount = 0;
            List<DNNGo_DNNGalleryPro_Files> fileList = DNNGo_DNNGalleryPro_Files.FindAll(qp, out RecordCount);
        ...
        public void PushPictureItem()
        {
            ...
            if (MediaID > 0)
            {
                ...
                DNNGo_DNNGalleryPro_Files PictureItem = DNNGo_DNNGalleryPro_Files.FindByKeyForEdit(MediaID);
            ...
            if (!String.IsNullOrEmpty(UrlValue) && UrlValue != "0")
            {
                if (UrlValue.IndexOf("FileID=", StringComparison.CurrentCultureIgnoreCase) == 0)
                {
                    ...
                    if (int.TryParse(UrlValue.Replace("FileID=", ""), out FileID) && FileID > 0)
                    {
                        ...
                        {
                            DefaultValue = string.Format("{0}{1}{2}", DNNGalleryPro_PortalSettings.HomeDirectory, fi.Folder, Server.UrlPathEncode(fi.FileName));
                        }
                    }
                }
        ...
        public void DeleteLayerItem()
        {
            ...
            if (SliderID > 0)
            {
                ...
                if (LayerID > 0)
                {
                    DNNGo_DNNGalleryPro_Layer Layer = new DNNGo_DNNGalleryPro_Layer();
                    Layer.ID = LayerID;

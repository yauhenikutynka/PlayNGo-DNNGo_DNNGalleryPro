...
namespace DNNGo.Modules.DNNGalleryPro
{
    public partial class Index_Manager : BasePage
    {
        ...
        public String Token
        {
            get { return WebHelper.GetStringParam(HttpContext.Current.Request, "Token", "ManagerList").ToLower(); }
        }
        ...
        private MenuTabItem _ActiveMenuTab = new MenuTabItem();
        /// <summary>
        /// 当前激活的菜单
        /// </summary>
        public MenuTabItem ActiveMenuTab
        {
            get
            {
                if (!(_ActiveMenuTab != null && !String.IsNullOrEmpty(_ActiveMenuTab.Token)))
                {
                    if( InitMenuTabCollection.ContainsKey(Token))
                    {
                        _ActiveMenuTab = InitMenuTabCollection[Token];
                    }
          
                }
                return _ActiveMenuTab;
            }
        }
        ...
        public MenuTabCollection InitMenuTabCollection
        {
            get
            {
                if (_InitMenuTabCollection == null || _InitMenuTabCollection.Count < 1)
                {
                    ...
                    Boolean IsDataList = true;

                    //如果没有分组的效果需要去掉分组
                    EffectDBEntity EffectDB = Setting_EffectDB;
                    if (!(EffectDB != null && String.IsNullOrEmpty(EffectDB.Name)))
                    {
                        if (!EffectDB.Groups) _InitMenuTabCollection.Remove("groups");
                        if (!EffectDB.Downloads) _InitMenuTabCollection.Remove("downloadlogs");

                        if (!EffectDB.Sliders)
                        {
                            _InitMenuTabCollection.Remove("addnew");
                            _InitMenuTabCollection.Remove("managerlist");
                            IsDataList = false;
                        }

                    }

                    //如果数据来自其他模块，需要屏蔽列表和媒体库
                    if (Settings_CopyOfOtherModule)
                    {
                        _InitMenuTabCollection.Remove("datalist");
                        _InitMenuTabCollection.Remove("library");
                        IsDataList = false;
                    }

                    if (!IsDataList && (Token == "managerlist" || Token == "addnew"))
                    {
                        Response.Redirect(xUrl("Effect_Options"));
                    }
